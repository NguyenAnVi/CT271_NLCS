    ...
    <div class="uk-child-width-expand@s uk-text-center uk-align-center uk-padding" uk-grid style="width: 900px;">
        <div class="uk-card uk-card-body uk-card-default uk-grid-margin uk-margin " uk-grid>
            ...
            <div class="uk-width-expand@s uk-width-2-3@m uk-padding">
                <form id="register-form" class="uk-panel uk-panel-box uk-form" method="POST" action="{{ route('admin.register') }}">
                    @csrf
                    <div class="uk-form-row uk-margin">
                        <input class="uk-input uk-width-1-1 uk-form-large @error('name') uk-form-danger @enderror" type="text" name="name" placeholder="Họ và tên" required>
                    </div>
                    @error('name')
                    ...
                    <div class="uk-form-row uk-margin">
                        <input class="uk-input uk-width-1-1 uk-form-large @error('phone') uk-form-danger @enderror" type="phone" name="phone" placeholder="Điện thoại" required>
                    </div>
                    @error('phone')
                    <span class="uk-text-danger">
                        <strong>{{ $message }}</strong>
                    ...
                    <div class="uk-form-row uk-margin">
                        <input class="uk-input uk-width-1-1 uk-form-large @error('password') uk-form-danger @enderror" type="password" name="password" placeholder="Nhập mật khẩu mới" required>
                    </div>@error('password')
                    <span class="uk-text-danger">
                        <strong>{{ $message }}</strong>
                    </span>
                ...
                <form id="login-form" action="{{ route('admin.login') }}" method="GET" style="display: none;"></form>
            </div>
        </div>
    </div>
@endsection
